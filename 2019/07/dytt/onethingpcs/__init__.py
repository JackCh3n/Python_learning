...
class otc_api:
    ...
    def login(self,user="",passwd=""): # 登陆
        try:
            login_data = common.body(deviceid=common.get_device_id(user), imeiid=common.get_imei_id(user), phone=user, pwd=common.get_pwd(passwd),account_type='4')
    ...
    def list_peer_info(self): # 对端信息
        try:
            peer_data = common.get_params(dict(appversion=config.APP_VERSION, v="8", ct="5"), self.user_info.get("sessionid"), True)
    ...
    def ansi_remote_download_url(self,url=False,peerid=False): # 解析远程下载的URL数据
        try:
            ...
            result = self.request_handler.post(config.URLRESOLVE_REMOTE_DOWNLOAD_URL + "pid={0}&v=2&ct=31&ct_ver={1}".format(peerid, config.APP_VERSION), data=post_data)
            if result.status_code == 200:
                temp = result.json()
                if temp.get("rtn") == 0:
                    del temp["rtn"]
    ...
    def create_remote_download_task(self,url,file_path=False,file_name=False,peerid=False):  # 创建远程下载任务
        try:
            ...
            result = self.request_handler.post(task_url+"pid={}&v=2&ct=31&ct_ver={}".format(peerid, config.APP_VERSION), headers = {
